#pragma once
// Principal.h — Página de USUARIO (mobile-first, estética base preservada)
...
<div class="wrap">
  <header>
    ...
    <div>
      ...
      <div class="sub" id="subtitle">Actualización en vivo</div>
    </div>
  </header>
...
<script>
  ...
  // ---- Render único (usado tanto por fetch inicial como por WebSocket) ----
  function render(d){
  ...
  // ---- WebSocket push + fetch inicial ----
  let ws;
  function connectWS(){
    const url = `ws://${location.hostname}:81/`;
    ws = new WebSocket(url);
    ws.onopen = ()=>{ if(sub) sub.textContent='Conectado (en vivo)'; };
    ws.onclose= ()=>{ if(sub) sub.textContent='Reconectando…'; setTimeout(connectWS, 1500); };
    ws.onmessage = (ev)=>{ try{ render(JSON.parse(ev.data)); }catch(e){} };
  }

  // Estado inicial vía HTTP (por si WS demora)
  fetch('/status',{cache:'no-store'}).then(r=>r.json()).then(render).catch(()=>{});
  connectWS();
</script>
</body>
</html>
