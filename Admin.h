#pragma once
// Admin.h — Panel de administración (MOBILE-FIRST)
// Muestra fase + contador, aforo, temperatura y humedad, ingresos/egresos y botón Reset Aforo (POST /reset)
...
<script>
  ...
  async function resetAforo(){
    ...
    try{
      ...
      if(!r.ok){
        // Fallback: algunos setups lo exponen en GET
        r=await fetch('/reset');
      }
